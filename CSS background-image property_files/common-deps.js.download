...
Util.objFieldOnSetCallback = function (params) {
  ...
  if (typeof scopeRef[fieldName] !== 'undefined') {
    lookupControlRef._completed = true;
...
Util.patchAllAuthLinksHelper = function (prs) {
  ...
  for (var i = 0; i < links.length; i++) {
    ...
    if (
      link.getAttribute('data-skip-href-patch') === 'true'
      || link.href === 'https://profile.w3schools.com/sign-up?redirect_url=https%3A%2F%2Fspaces.w3schools.com%2F'
    ) {
      continue;
    }

    if (link.href.indexOf(searchLogInUrl) === 0) {
      link.href = replaceLogInUrl;
      match = true;
    } else if (link.href.indexOf(searchSignUpUrl) === 0) {
      link.href = replaceSignUpUrl;
      match = true;
    }

    if (match) {
      Util.logDebug('_patchAllAuthLinksHelper -> link.href: ', link.href);
    }
  }
};
...
Util.patchAuthLinks = function (prs) {
  ...
  if (loggedIn) {
    ...
    Util.patchAllAuthLinksHelper({
      searchLogInUrl: 'https://profile.w3schools.com/log-in',
      replaceLogInUrl: 'https://profile.w3schools.com/log-in?redirect_url=' + encodeURIComponent(currentUrl),
      searchSignUpUrl: 'https://profile.w3schools.com/sign-up',
      replaceSignUpUrl: 'https://profile.w3schools.com/sign-up?redirect_url=' + encodeURIComponent(currentUrl),
    });
  }
};
...
Util.getUrlParsed = function (url) {
  ...
  Util.logDebug('getUrlParsed -> output: ', output);
